...
const Crown: React.FC<CrownProps> = ({ index }: CrownProps) => {
  // State (redux)
  const hasCrown = useSelector((state: AppState) => {
    const scoreList = scoreListSelector(state);
    const score = playerListSelector(state)[index].score;
    return score >= Math.max(...scoreList) && score > 0;
  });
  ...
  return (
    <svg
      ...
      viewBox="0 0 44 35"
      className={hasCrown ? "crown has-crown" : "crown"}
    >
      <path
